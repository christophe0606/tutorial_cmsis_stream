...
template<>
std::array<uint16_t,1> ProcessingNode<float,128>::selectors = {SEL_NUMBER_ID};
std::array<uint16_t,1> Sink::selectors = {SEL_NUMBER_ID};
...
typedef struct {
    ProcessingNode<float,128> *processing;
...
int init_scheduler(void *evtQueue_)
{
    ...
    nodes.sink = new (std::nothrow) Sink;
    if (nodes.sink==NULL)
    {
        return(CG_MEMORY_ALLOCATION_FAILURE);
    }
...
/* Subscribe nodes for the event system*/
    nodes.processing->subscribe(0,*nodes.sink,0);
